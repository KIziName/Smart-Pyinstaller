    ...
    def _build_ui(self):
        ...
        for i, (name, label) in enumerate(MODULES):
            ttk.Checkbutton(opts, text=label,
                ...
                row=i + 1, column=0, sticky=tk.W, padx=(6, 24), pady=2)

        for text, var, r, c in (
            ("Keep .spec file", self.keep_spec_var, 1, 1),
            ("Keep build/ folder", self.keep_build_var, 2, 1),
            ("Show console window  (--noconsole: not for Linux)",
             self.console_var, 1, 2),
            ("Request admin privileges", self.admin_var, 2, 2),
            ("Clean before build (--clean)", self.clean_build_var, 3, 2),
        ):
        ...
        self.detected_label.grid(row=len(MODULES) + 1, column=0, columnspan=3,
