...
class SmartPyInstallerGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.smart_excludes_var = tk.BooleanVar(value=True)
    ...
    def _build_ui(self):
        ...
        ttk.Label(opts, text="Output files", font=BOLD_FONT).grid(
            row=0, column=0, sticky=tk.W, padx=6, pady=(0, 4))
        ttk.Label(opts, text="Application", font=BOLD_FONT).grid(
            row=0, column=1, sticky=tk.W, padx=6, pady=(0, 4))
        ttk.Label(opts, text="Optimization", font=BOLD_FONT).grid(
            row=0, column=2, sticky=tk.W, padx=6, pady=(0, 4))

        for text, var, r, c in (
            ("Keep .spec file", self.keep_spec_var, 1, 0),
            ("Keep build/ folder", self.keep_build_var, 2, 0),
            ("Show console window  (--noconsole: not for Linux)",
             self.console_var, 1, 1),
            ("Request admin privileges", self.admin_var, 2, 1),
            ("Clean before build (--clean)", self.clean_build_var, 3, 1),
            ("Exclude unused libraries  (AST-checked, smaller EXE/ELF)",
             self.smart_excludes_var, 1, 2),
        ):
        ...
        ).grid(row=4, column=0, columnspan=3, sticky=tk.W, padx=6, pady=(8, 0))
    ...
    def _scan_project(self):
        ...
        if not self.base_dir.is_dir():
            ...
            self._log(f"{PREFIX_ERROR} Folder does not exist: {self.base_dir}",
                      TAG_ERROR)
            return
        ...
        if self.used_imports:
            preview_list = sorted(self.used_imports)
            preview = ', '.join(preview_list[:8])
            more = '' if len(preview_list) <= 8 else \
                f" (+{len(preview_list) - 8} more)"
            self._log(f"Imports detected: {preview}{more}", TAG_MUTED)

            protected = sorted(set(COMMON_EXCLUDES) & self.used_imports)
            if protected:
                self._log(
                    f"Will keep (used by project): {', '.join(protected)}",
                    TAG_MUTED,
                )
    ...
    def _start_build(self):
        ...
        options = BuildOptions(
            ...
            smart_excludes=self.smart_excludes_var.get(),
        )
    ...
    def _run_build(self, base_dir: Path, script: Path, exe_name: str,
        ...
        if options.smart_excludes:
            ...
            for mod in to_exclude:
                cmd.append(f"--exclude-module={mod}")

            if to_exclude:
                self.q_info(
                    f"Excluded {len(to_exclude)} unused libraries "
                    "(smaller EXE/ELF)")
            if skipped:
                self.q_warn(
                    "Not excluding (used by project): "
                    + ", ".join(sorted(skipped)))
        else:
            self.q_info("Smart excludes disabled — PyInstaller will bundle "
                        "everything it finds")
