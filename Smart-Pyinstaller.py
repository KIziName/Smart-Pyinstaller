...
class SmartPyInstallerGUI:
    ...
    def _build_ui(self):
        ...
        for text, var, r, c in (
            ...
            ("Exclude unused libraries  (smaller EXE/ELF)",
        ...
        ttk.Label(
            opts,
            text="Excludes: PyQt/PySide, numpy, scipy, pandas, matplotlib, "
                 "Jupyter, pytest, PIL/setuptools/wheel, …",
    ...
    def _run_build(self, base_dir: Path, script: Path, exe_name: str,
        ...
        if options.smart_excludes:
            ...
            self.q_info(
                f"Excluded {len(COMMON_EXCLUDES)} unused libraries "
                "(smaller EXE/ELF)")
        else:
